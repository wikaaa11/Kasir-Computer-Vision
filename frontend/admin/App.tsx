...
const App: React.FC = () => {
  ...
  const normalizeOrderRecord = (order: any) => {
    ...
    const discount = Number(
      order.discount ?? order.voucher_discount ?? order.points_used ?? 0
    );

    const paymentMethod = String(
      order.payment_method ?? order.metode_bayar ?? 'QRIS'
    ).trim();

    const customerType = String(
      order.tipe_pelanggan ?? order.customer_type ?? 'GUEST'
    ).trim();

    const customerName = String(
      order.nama_pelanggan ?? order.customer_name ?? 'Guest'
    ).trim();
    ...
    return {
      ...
      harga: Number(
        firstItem?.price_snapshot ?? firstItem?.price ?? order.price ?? subtotal
      ),
      qty: totalQty,
      total,
      subtotal,
  ...
  const fetchOrdersFromBackend = async () => {
    const response = await withTimeout(
      `${BACKEND_URL}${CV_API_PREFIX}/orders?order_type=${encodeURIComponent(
        CV_ORDER_TYPE
      )}`,
      undefined,
      12000
    ...
    if (!response.ok) {
      throw new Error(
        await pickApiErrorMessage(
          response,
          'Gagal memuat order dari backend pusat.'
        )
      );
    }
    ...
    return rawOrders
      ...
      .filter(
        (order: any) =>
          order &&
          String(order.order_type || CV_ORDER_TYPE).toLowerCase() ===
            CV_ORDER_TYPE
      );
  };
  ...
  const normalizeProduct = (product: any) => {
    const id = String(
      product.product_code ?? product.code ?? product.id ?? ''
    ).trim();
    ...
    return {
      ...
      cashback_reward: Number(
        product.points ?? product.cashback_reward ?? product.poin ?? 0
      ),
      category_name: categoryName,
      category_code: String(
        product.category_code ?? product.categoryCode ?? categoryName
      ).trim(),
      category_id: Number(product.category_id ?? 1),
      product_type:
        String(product.product_type ?? 'cv').trim().toLowerCase() || 'cv',
    ...
    try {
      ...
      if (results[0].status === 'fulfilled' && results[0].value.ok) {
        ...
        const productTypeLabel =
          role === 'kiosk_admin' ? 'kiosk' : role === 'cv_admin' ? 'cv' : null;

        const filtered = productTypeLabel
          ? normalizedProducts.filter(
              (p: any) =>
                String(p.product_type || '').toLowerCase() === productTypeLabel
            )
          : normalizedProducts;
  ...
  const toProductPayload = (product: any) => {
    ...
    const resolvedProductType =
      String(product.product_type || (role === 'kiosk_admin' ? 'kiosk' : 'cv'))
        .trim()
        .toLowerCase() || 'cv';
  ...
  const handleCreateProduct = async (newProduct: any) => {
    try {
      const response = await withTimeout(
        `${BACKEND_URL}${CV_API_PREFIX}/products`,
        {
          method: 'POST',
      ...
      if (!response.ok) {
        const message = await pickApiErrorMessage(
          response,
          'Gagal menyimpan produk ke backend.'
        );
  ...
  const handleUpdateProduct = async (
    ...
    try {
      ...
      const fromList = allProducts.find(
        (p: any) => String(p.id ?? p.code ?? '') === String(productCode)
      )?.db_id;

      const productId = String(
        updatedProduct?.db_id ?? fromList ?? fallback
      ).trim();

      const response = await withTimeout(
        `${BACKEND_URL}${CV_API_PREFIX}/products/${encodeURIComponent(productId)}`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const renderContent = () => {
      ...
      return (
        <div className="h-full flex flex-col items-center justify-center p-8 text-center space-y-6">
          ...
          <button
            ...
            className="px-6 py-2.5 bg-orange-600 text-white rounded-xl font-bold flex items-center gap-2 mx-auto hover:bg-orange-700 transition-all"
          >
            <RefreshCw size={18} />
    ...
    switch (activeTab) {
      ...
      case AdminTab.TRANSACTIONS:
        return <Transactions transactions={transactions} onRefresh={fetchAllData} />;
  ...
  if (!user) {
    return (
      <Login
        onLogin={(loggedInUser: AdminUser) => {
    ...
    <div className="flex h-screen bg-white overflow-hidden text-slate-900">
      ...
      >
        <div className="p-6 flex items-center justify-between">
          ...
          >
            <div className="w-9 h-9 rounded-xl overflow-hidden bg-white border border-slate-200 shadow-sm flex items-center justify-center">
              <img
                src="/logo.jpeg"
                alt="Vision Admin Logo"
        ...
        <nav className="flex-1 px-4 space-y-1.5 mt-4">
          {[
            ...
            {
              id: AdminTab.INVENTORY,
              icon: Package,
              label: 'Inventory',
            },
            {
              id: AdminTab.TRANSACTIONS,
              icon: History,
              label: 'Transactions',
            },
          ]
        ...
        <div className="p-4">
          <div
            className={`rounded-[24px] border border-orange-100 bg-white shadow-[0_12px_30px_rgba(15,23,42,0.06)] ${
          ...
          >
            <div className="space-y-3">
              <div
                ...
                } rounded-2xl bg-orange-500 px-3 py-3 shadow-[0_10px_24px_rgba(249,115,22,0.22)]`}
              >
                  ...
                  {isSidebarOpen && (
                    <div>
                      ...
                      <p className="text-xs font-black text-white">
                        {dbConnected && !error ? 'Connected' : 'Offline'}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <button
                ...
                className="group flex w-full items-center justify-center gap-2 rounded-2xl bg-red-500 py-3 font-black text-white transition-all hover:bg-red-600 hover:shadow-lg hover:shadow-red-200"
              >
                <LogOut
                  size={18}
